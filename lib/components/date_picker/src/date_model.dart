...
import 'package:anad_magicar/components/date_picker/src/date_format.dart';
...
class DatePickerModel extends CommonPickerModel {
  ...
  DatePickerModel(
    ...
    if(locale==LocaleType.fa) {
      Jalali jalali=Jalali.fromDateTime(this.currentTime);
      this.jCurrentTime=jalali;
      }
    _fillLeftLists();
    _fillMiddleLists();
    _fillRightLists();
    ...
    int minDay = _minDayOfCurrentMonth();
      if(locale==LocaleType.fa) {
        Jalali jalali=Jalali.fromDateTime(this.currentTime);
        Jalali minJalali=Jalali.fromDateTime(this.minTime);

        _currentLeftIndex = jalali.year - minJalali.year;
        _currentMiddleIndex = jalali.month - minMonth;
        _currentRightIndex = jalali.day - minDay;
      }
      else {
  ...
  void _fillLeftLists() {
    this.leftList = List.generate(maxTime.year - minTime.year + 1, (int index) {
      ...
      if(locale==LocaleType.fa){
        Jalali jalali=Jalali.fromDateTime(minTime);
        return '${jalali.year + index}${_localeYear()}';
      }
  ...
  int _maxMonthOfCurrentYear() {
    if(locale==LocaleType.fa){
      Jalali jalali=Jalali.fromDateTime(maxTime);
      return currentTime.year == maxTime.year ? jalali.month : 12;
    }
  ...
  int _minMonthOfCurrentYear() {
    if(locale==LocaleType.fa){
      Jalali jalali=Jalali.fromDateTime(minTime);
      return currentTime.year == minTime.year ? jalali.month : 1;
    }
    return currentTime.year == minTime.year ? minTime.month : 1;
  }

  int _maxDayOfCurrentMonth() {
    int dayCount = calcDateCount(currentTime.year, currentTime.month,locale);
      if(locale==LocaleType.fa) {
        Jalali jalali=Jalali.fromDateTime(maxTime);
    ...
    if (currentTime.month == 2 && currentTime.day == 29) {
      newTime = currentTime.isUtc
          ? DateTime.utc(
              destYear,
              currentTime.month,
              calcDateCount(destYear, 2,locale),
            )
          : DateTime(
              destYear,
              currentTime.month,
  ...
  void setMiddleIndex(int index) {
    ...
    DateTime newTime;
    //change date time
    int dayCount = calcDateCount(currentTime.year, destMonth,locale);
    newTime = currentTime.isUtc
        ? DateTime.utc(
            currentTime.year,
            destMonth,
