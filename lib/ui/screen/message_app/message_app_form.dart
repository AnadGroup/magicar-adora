              ...
              return FutureBuilder<List<ApiMessage>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasData && snapshot.data != null) {
                    ...
                    Map<int, List<Map<String, dynamic>>> recIds =
                        groupBy(mapList, (o) => o['SenderUserId']);
                    loadGroupedMessages(recIds);
                    return StreamBuilder<Message>(
                      stream: updateListMessageNoty.noty,
                      builder: (context, snapshot) {
                        List<int> deletedIds = List();
                        if (snapshot.hasData && snapshot.data != null) {
                          var msg = snapshot.data;
                          if (msg.type == "DELETED_SUCCEED_EMPTY_LIST") {
                            int senderId = msg.id;
                            if (recIds != null && recIds.isNotEmpty) {
                              recIds
                                ..removeWhere((key, value) => key == senderId);
                            }
                          } else if (msg.type == 'DELETED_SUCCEED') {
                            if (msg.Ids != null && msg.Ids.isNotEmpty) {
                              deletedIds..addAll(msg.Ids);
                            }
                          }
                        }
                        return recIds == null || recIds.isEmpty
                            ? NoDataWidget(
                                noCarCount: false,
                              )
                            : Padding(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      ApiMessage apiMessage =
                                          ApiMessage.fromMap(newList[0]);
                                      List<ApiMessage> apiMsgs = newList
                                          .map<ApiMessage>(
                                              (m) => ApiMessage.fromMap(m))
                                          .toList();
                                      if (deletedIds != null &&
                                          deletedIds.isNotEmpty) {
                                        for (var msgId in deletedIds) {
                                          apiMsgs.removeWhere((element) =>
                                              element.MessageId == msgId);
                                        }
                                      }
                                      var uri = apiMsgs
                                          .where((m) =>
                                              m.MessageStatusConstId !=
                                              ApiMessage
                                                  .MESSAGE_STATUS_AS_READ_TAG)
                                          .toList();
                                      unReadItems =
                                          uri == null ? 0 : uri.length;

                                      return MessageAppItem(
                                        carId: widget.carId,
                                        message: apiMessage,
...
NotyBloc<Message> updateListMessageNoty = NotyBloc();
