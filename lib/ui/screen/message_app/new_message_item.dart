...
class _NewMessageItemState extends State<NewMessageItem> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      if(widget.detailVM.messages!=null && widget.detailVM.messages.isNotEmpty) {
        int index=0;
        for (ApiMessage msg in widget.detailVM.messages) {
          if (msg.MessageStatusConstId != ApiMessage.MESSAGE_STATUS_AS_READ_TAG) {
      ...
      if (result != null) {
        if (result.IsSuccessful) {
          ...
          statusMessageNoty.updateValue(
              new Message(type: 'MSG_READ', index: index));
          CenterRepository.messageCounts = CenterRepository.messageCounts - 1;
          messageCountNoty.updateValue(
              new Message(index: CenterRepository.messageCounts));
        }
        else {
  ...
  Widget build(BuildContext context) {
      ...
      builder: (context,snapshot)
    {
      ...
      return
        new Padding(padding: EdgeInsets.only(top: 5.0, right: 10.0, left: 10.0),
          child:
          new Column(
            //mainAxisAlignment: MainAxisAlignment.end,
            children: <Widget>[
              new Padding(
                padding: EdgeInsets.only(top: 30.0, bottom: 20.0, left: 0.0),
                child:
                Row(
                  ...
                  hasSelected ?  Container(
                      width: 28,
                      height: 28.0,
                      child:
                      FlatButton(
                        onPressed: () async {
                        var result=await  restDatasource.deleteUserMessage(selectedIds);
                        if(result!=null &&
                        result.IsSuccessful) {
                          centerRepository.showFancyToast(
                              'عملیات حذف با موفقیت انجام شد', true);
                          for (var i = 0; i < selectedIds.length; i++)
                            widget.detailVM.messages.removeWhere((e) =>
                            e.MessageId == selectedIds[i]);
                              selectedIds..clear();
                          deleteMessageNoty.updateValue(
                              Message(type: 'DELETED_SUCCEED',));
                        }
                        else{
                          centerRepository.showFancyToast('عملیات حذف با موفقیت انجام نشد', false);
                        }
                        },
                        child: Icon(
                        ...
                        return
                          Card(
                            margin: new EdgeInsets.only(
                                left: 1.0, right: 1.0, top: 8.0, bottom: 5.0),
                            ...
                            GestureDetector(
                              onTap: () {
                                changeSmsStatus(
                                    widget.detailVM.messages[index], index);
                              },
                              child:
                              new Container(
                                              ...
                                              if (value) {
                                                ...
                                                if (!selectedIds.contains(
                                                  ...
                                                  if(selectedIds!=null && selectedIds.isNotEmpty){
                                                    deleteMessageNoty.updateValue(Message(type: 'ITEM_SELECTED',status: true));
                                                  }
                                                }
                                              }
                                              else {
                                                if (selectedIds != null &&
                                                  ...
                                                  if (selectedIds.contains(
                                                    ...
                                                    if(selectedIds==null || selectedIds.isEmpty){
                                                      deleteMessageNoty.updateValue(Message(type: 'ITEM_SELECTED',status: false));
                                                    }else{
                                                      deleteMessageNoty.updateValue(Message(type: 'ITEM_SELECTED',status: true));
                                                    }
                                                  }
