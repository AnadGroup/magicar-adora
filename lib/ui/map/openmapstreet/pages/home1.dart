...
class MapPageState extends State<MapPage> with AutomaticKeepAliveClientMixin {
  ...
  locator.Position _lastKnownPosition;
  locator.Position _currentPosition;
    ...
    RxBus.register<ChangeEvent>().listen((ChangeEvent event) {
      if (event.type == 'GPS_STATUS') {
        /* FlashHelper.informationBar(context,
               duration:  Duration(seconds: 4), message: event.message);*/

        /*FlashHelper.actionBar(context,
            duration: new Duration(seconds: 10),
            title: 'تنظیمات GPS',
            message: event.message,
            primaryAction: new FlatButton(onPressed: () async {},
              child: Button(color: Colors.pinkAccent.value,
                backTransparent: true,
                clr: Colors.pinkAccent,
                fixWidth: true,
                wid: 100,
                title: 'تنظیمات',),), onPrimaryActionTap: null);*/
      }
    });
  }

  Future<locator.GeolocationStatus> checkGPSStatus() {
...
*/
  ...
  _initCurrentLocation() {
    ...
    geolocator
      ..getCurrentPosition(
        ...
        if (position != null) {
          currentLocation = getLocationDataFromPosition(position);
        }
  ...
  getLocationDataFromPosition(locator.Position position) {
  ...
  void initState() {
    ...
    pcarsPairedActivated = new List();
    carInfoss = getCarInfo(false);

  if(!kIsWeb) { liveMapController = LiveMapController(
      autoCenter: true,
      mapController: mapController,
      verbose: true,
      autoRotate: true,
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      checkGPSStatus().then((value) {
        if ((value == locator.GeolocationStatus.denied ||
            value == locator.GeolocationStatus.disabled)) {
  ...
  Future<loc.LocationData> getCurrentLoaction() async {
    ...
    _initCurrentLocation();
    /* var locationOptions = locator.LocationOptions(accuracy: locator.LocationAccuracy.high, distanceFilter: 100,);

    locator.Position position = await locator.Geolocator().getCurrentPosition(desiredAccuracy: locator.LocationAccuracy.high);
    if(position!=null) {
      currentLocation=LocationDataLocator.fromMap({'latitude' :position.latitude,
      'longitude' : position.longitude,
      'speed' :position.speed,
      'accuracy' : position.accuracy,
      'altitude': position.altitude,
    ...
    StreamSubscription<locator.Position> positionStream = geolocator.getPositionStream(locationOptions).listen(
            (locator.Position position) {
...
*/
    /*try {
      if (CenterRepository.GPS_STATUS ||
          CenterRepository.GPS_STATUS_CONFIRMED) {
        ...
        centerRepository.checkGPSDeviceStatus(context);
      }
      } catch (e) {
...
if(!kIsWeb) {
                          ...
                          children: <Widget>[
                            ...
                            /* Padding(padding: EdgeInsets.only(bottom: 5.0),
                                            ...
                                            new GestureDetector(
                                              onTap: () {
                                                if(lastCarIdSelected==null || lastCarIdSelected==0){
                                                  centerRepository.showFancyToast('لطفا ابتدا خودرو را انتخاب نمایید',false);
                                                }else {
                                                  _formKey.currentState.save();
                                                  forAnim=true;
                                                 lines2= processLineData(
  ...
  Widget build(BuildContext context) {
    ...
    return /*FutureBuilder<locator.GeolocationStatus>(
      future: checkGPSStatus(),
      builder: (context, snapshot) {
        if (snapshot.hasData && snapshot.data != null) {
          if (snapshot.data == locator.GeolocationStatus.denied ||
              snapshot.data == locator.GeolocationStatus.disabled) {
            if (!gpsDialogGasShownBefore) {
              gpsDialogGasShownBefore = true;
              CenterRepository.GPS_STATUS_CONFIRMED = false;
              CenterRepository.GPS_STATUS = false;
              RxBus.post( ChangeEvent(
                  type: 'GPS_STATUS',
                  message:
                      'GPS غیرفعال و یا غیرقابل دسترسی می باشد.لطفا بررسی کنید'));
            }
          } else {
            CenterRepository.GPS_STATUS_CONFIRMED=true;
            CenterRepository.GPS_STATUS=true;
        ...
        return*/ /*FutureBuilder<locator.GeolocationStatus>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData && snapshot.data != null) {
              if (snapshot.data == locator.GeolocationStatus.disabled ||
                  snapshot.data == locator.GeolocationStatus.denied) {
                if (!gpsDialogGasShownBefore) {
                  gpsDialogGasShownBefore = true;
                  CenterRepository.GPS_STATUS_CONFIRMED = false;
                  CenterRepository.GPS_STATUS = false;
              ...
              return Scaffold(
                ...
                body:
                    /* ExtendedNavigationBarScaffold(
                            ...
                            onTap: (index) {
                              if (index == 2) {
                                _showBottomSheetSearchCarForJoin(context);
                              }
                            },
